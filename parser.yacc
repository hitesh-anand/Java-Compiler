...
DoStatement:
    ...
    $$->ir_label = lab;
    Quadruple* quad = new Quadruple($$->ir_label);
    $$->code.push_back(quad);
    ircode.push_back(quad);
    for(auto quad : $2->code)
    {
        $$->code.push_back(quad);
    }   
    $5->True = "L"+to_string(labelCnt++);
    $5->False = $2->next = $$->next;
    string cod = $5->varName+" = 0";
    Quadruple* jmp = new Quadruple(2, cod, $$->False);
    $$->code.push_back(jmp);
    ircode.push_back(jmp);
    jmp = new Quadruple(3, cod, $$->ir_label);
    $$->last = ircode.size() - 1;
    $$->code.push_back(jmp);
    processPostIncre($$);
}
